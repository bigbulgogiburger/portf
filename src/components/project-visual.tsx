...
export function ProjectVisual({id, eager=false}:{id:string; eager?:boolean}) {
  ...
  return <div className={`project-art process-art ${harness?"harness-art":"agent-art"}`} aria-label={harness?"검증한 코드와 커밋 대상 비교 흐름":"접수 초안 생성과 사용자 확인 후 접수 흐름"}>
    <div className="process-heading">{harness?"jira-harness":"CS AI Agent"}<span>{harness?"코드 검증 흐름":"신규 접수 흐름"}</span></div>
    <div className="process-steps">{(harness?["테스트·리뷰","Git tree 기록","커밋 대상 비교"]:["권한 내 조회","접수 초안","확인 후 접수"]).map((s,i)=><div key={s}><strong>{s}</strong>{i<2&&<ArrowRight size={18}/>}</div>)}</div>
